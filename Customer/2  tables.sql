...
--AS So what should be changed? null?
--cel constraint should be not a negative num
    AmountPurchased2022 int null constraint ck_Customer_AmountPurchased2022_must_be_equal_or_greater_than_200 check(AmountPurchased2022 >= 200),
    Terms varchar(6) not null constraint ck_Customer_terms_must_be_COD_Net15_Net30_Net60 check(Terms in ('COD', 'Net 15', 'Net 30', 'Net 60')),
    TotalPaid int not null constraint ck_Customer_TotalPaid_must_be_greater_than_zero check(TotalPaid > 0),
    CustomerCode as concat(substring(ShopName, 1, 3), substring(FirstName, 1, 3), right(PhoneNumber, 4))
    constraint u_Customer_CustomerCode unique
>>>>>>> 722db361edc0075854ac2cb090fdd1a62089294f
)
go
